    ...
    for b in range(4):
        ...
        if block_filters == []:
            ...
            for num in block_filters:
                if first_ind:
                    x = layers.Conv2D(num, (3, 3), activation='relu', padding='same', name=block_name+'_conv'+str(cnt))(img_input)
                    cnt += 1
                    first_ind = False
                else:
                    x = layers.Conv2D(num, (3, 3), activation='relu', padding='same', name=block_name+'_conv'+str(cnt))(x)
                    cnt += 1
